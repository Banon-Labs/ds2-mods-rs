...
test_deny_corpus_1_retraction_with_no_action if {
	"DS2-MODS-NO-STALL-ON-FRICTION" in halted_on({
		"friction": "shut up",
...
test_deny_corpus_4_decision_handback_despite_having_acted if {
	"DS2-MODS-NO-DECISION-HANDBACK" in halted_on({
		"friction": "*you*",
		"handback": "your call",
		"acted": "1",
	})
...
test_corpus_4_does_not_also_trip_the_stall_arm if {
	not "DS2-MODS-NO-STALL-ON-FRICTION" in halted_on({
		"friction": "*you*",
		"handback": "your call",
		"acted": "1",
	})
...
test_deny_blame_deflection_without_ownership if {
	"DS2-MODS-NO-BLAME-DEFLECTION" in halted_on({
...
test_stall_reason_demands_action_and_protects_the_admission if {
	...
	some d in halts
	d.rule_id == "DS2-MODS-NO-STALL-ON-FRICTION"
...
test_handback_reason_demands_choosing_and_closes_the_acted_excuse if {
	...
	some d in halts
	d.rule_id == "DS2-MODS-NO-DECISION-HANDBACK"
...
test_blame_reason_keeps_reporting_required_and_demands_cause if {
	...
	some d in halts
	d.rule_id == "DS2-MODS-NO-BLAME-DEFLECTION"
...
test_halt_on_object_shaped_signal if {
	...
	"DS2-MODS-NO-STALL-ON-FRICTION" in rule_ids(halts)
}
