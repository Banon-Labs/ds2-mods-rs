...
#
# PORTED FROM ../er-mods-rs (scripts/cupcake-hook.sh) on 2026-09-21, replacing this repo's
# earlier copy, which carried only bugs 1 and 2 below and passed --global-config a FILE path --
# bug 4, which made it load no global policy for its whole life while reporting success. It is
# wired to every cupcake event in .claude/settings.json, not to Stop alone as the old copy was;
# the newline repair in bug 3 exists for the command-text guards on PreToolUse and is the reason
# that matters. A fifth fix, --wasm-max-memory, is below run_cupcake and came from the user's
# global hook, which had it while neither repo hook did.
#
...
# fourth on 2026-08-31 -- all in er-mods-rs, against the same binary this repo runs:
#
...
#    green. That is why the mode is normalised here rather than waited on: an unrecognised mode
#    must degrade to "evaluate anyway", never to "evaluate nothing".
#
...
# WASM LINEAR MEMORY. The engine's WASM module runs out of memory on an ordinary long command,
# and an evaluation that runs out of memory is not a refusal -- it returns `{}` at exit 0, so every
# policy goes silent at once and the command is ALLOWED. `--wasm-max-memory` defaults to 10MB and
# the engine's ceiling is 100MB. The measurement is recorded in ~/.claude/hooks/cupcake-hook.sh
# (cupcake 0.5.2, bisecting the length of a `cat > doc.md <<'EOF' ... EOF` artifact write): the
# first command that aborts is 11,103 bytes at the default and 127,735 at the ceiling. Ten
# kilobytes is an ordinary handoff document or a long commit message, so the default fails the
# whole guard layer open on work this repo does every session. The global hook carried this fix
# while neither repo hook did; CUPCAKE_WASM_MAX_MEMORY still overrides it for a measurement.
#
# This is the same failure mode as the `walk()` dispatcher in .cupcake/system/evaluate.rego, from
# the other end: there the traversal is what exhausts the heap, here it is the payload size, and
# in both cases the symptom is `{}` at exit 0 rather than an error anybody sees.
