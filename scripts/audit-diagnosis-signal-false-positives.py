...
audits the three added since 2026-09-09, either separately or together:
  ...
  promissory (`DS2-MODS-NO-PROMISSORY-CLOSER`) refuses a turn that closes by announcing work in the
  ...
  unread (`DS2-MODS-NO-PROMISSORY-CLOSER`, second shape) refuses a turn that names the evidence
  ...
  handback (`DS2-MODS-NO-ZERO-INFORMATION-STOP`) refuses a turn whose ideal next user reply is
  ...
  deferral (`DS2-MODS-NO-DEFERRED-INVESTIGATION`) refuses a turn that names the agent's own next
...
turns; adjacent boundaries can report one turn more than once, so the boundary rate errs high).
  ...
  promissory  12 halts (0.32% of boundaries) = 7 distinct turns, 10 matched-and-exempt because the
              ...
              `DS2-MODS-NO-UNBACKED-CLAIM` exists to catch.
