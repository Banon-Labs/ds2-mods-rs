 ...
 *
 * THE POINT IS SHARING, not just warmth. One process takes the project lock and serves a TCP
 * port, so every agent that connects gets the same program concurrently. Without this, each
 * `query.sh` run takes its own exclusive lock and a second agent is simply locked out -- which
 * is exactly what happened during ds2-mods-rs-3rr, when a background survey agent held the
 * project and the title-flow trace had to be done with objdump instead.
 *
...
public class MCPServeHeadless extends GhidraScript {
	...
	protected void run() throws Exception {
		...
		// REFLECTION (see docs/GHIDRA-MCP.md): resolve the EXTENSION class
