...
Two things keep it off ordinary work, and both are structural rather than a judgement about intent:
  ...
  * the narration has to be the last sentence of that closing prose. A narration with a clause hung
    ...
    single use of the item." -- belongs to `DS2-MODS-NO-ZERO-INFORMATION-STOP`, which was written
...
def sentences(text: str) -> list[str]:
    ...
    The semicolon is what separates this rule from `DS2-MODS-NO-ZERO-INFORMATION-STOP`: its
...
# The gerunds `DS2-MODS-NO-PROMISSORY-CLOSER` already owns, copied from `CLOSER_VERBS` in
