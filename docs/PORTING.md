...
- **Multiplayer.** `er-invasion-warp` (`CSAutoInvadePoint`) and `er-seamless-bugfixes`
  (Seamless Co-op specific) have no DS2 analogue. **`er-invasion-path` was on this list and
  should not have been** -- see the correction below.
...
### Correction, 2026-09-22: `er-invasion-path` was in Tier 3 and should not have been

This document filed it under "Multiplayer ... no DS2 analogue (Havok-AI navmesh)". The premise
was half right and the conclusion was wrong, and the difference was one `strings` run.

There is no Havok AI in this engine -- the Havok here is `hk_2013.2` / `hk_2014.1.0-r1`,
animation and physics. But DARK SOULS II ships **its own** navigation stack, RTTI-named
throughout, and `scripts/ds2-rtti-vtables.py` maps every one of them to a vtable in a second:
...
DS2. The one that got checked against the DS2 binary moved two tiers. `er-invasion-warp` and
`er-seamless-bugfixes` are still sitting there **unchecked**, and this is the note that says
their placement is an assumption rather than a finding.

Two more corrections the same work produced, both about trusting names:

- **The Ghidra project's field labels are leads, not facts.** `CharacterManager+0x50` is named
  `player_ctrl` and is an array base -- `0x14035b6c5` stores `[manager + index*8 + 0x50] = chr`.
  Read as a pointer it usually answers with a real character, which is exactly why it survives
  being tried once.
- **Two classes in one family can disagree about their own layout.** `CameraOperator`'s
  constructor puts the view matrix at `+0x10`; the object the game actually draws through keeps
  it at `+0x20`. That was measured in game, not derived, and the static derivation had been
  checked against two call sites before it turned out to describe a different class.
