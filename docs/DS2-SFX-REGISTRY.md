...
4. **Negative cache.** If the resolver reports not-found, `0x140a0aa30` inserts an
   ...
   instances, `0x140a09d50` despawns them. The miss also goes into the map at `sys+0x2b8`.
...
- **Id range:** above 9999 skips the remaster probe (`id + 20000` / `+ 10000`,
  ...
  every area bundle).
- **Lifetime:** nothing but the mod references the resource, so area changes do not reach it. The
  `sys+0x30` name table is not joined, and whether a purge of zero-ref resources by timestamp
  exists was not read; holding the refcounts at `+0x10`/`+0x14` above zero is the guard **[inf]**.
...
- **Thread:** spawn and the lazy parse run on the caller's thread, which for the mod is the game
  thread **[read]** for the call chain. Which thread runs `0x140af6e70` (async file completion) was
  not traced; the buffer path does not use it.
