...
### `SAVE_DIR_BUILD` is called per request, which is what the in-session version would need

Worth recording because it is the one fact that says the in-session version is possible. The
function's two callers are both `SaveLoadSystem` methods:

* `FUN_1402e6230_saveLoadSetup__` at `0x1402e635c` -- setup;
* `FUN_1402e67f0` at `0x1402e6930` -- which opens

```asm
0x1402e6810:  mov  eax,[rcx+0x8]      ; the request state
              cmp  eax,0x6
              ja   <bail>
              mov  ecx,0x6a           ; 0b1101010 -- bits 1, 3, 5, 6
              bt   ecx,eax
              jae  <bail>
```

so it runs per request, for states `{1, 3, 5, 6}`. A directory built per request is a directory that
can differ per request -- **if those four states can be split into saves and loads**, the detour can
answer the game's own folder for a save and the staged copy for a load, and the whole handoff
disappears. Decoding them is `ds2-mods-rs`'s filed next step for this feature; nothing above depends
on it, and nothing above should be taken as evidence for it.
