...
/// Give the quit tab a fourth cell, using a row its own layout already contains.
///
/// The namer ships with `{0x1eacc9, 0x1eacca, 0x1eace9, 0, 0, 0}` in a six-slot array and a loop
/// that already runs six times, so three slots are spare by construction. The layout is not short
/// either: `FeSceneInGameMenu`'s element cache resolves FIVE cell-shaped children under this tab,
/// two of which ([`ds2_rva::FE_QUIT_TAB_SPARE_CELL_IDS`]) no namer ever names.
///
/// So this appends one of them, with the same two calls the original makes three times on the way
/// in. **Nothing is authored and nothing is patched** -- a row the game shipped and never used is
/// put on the list it was always eligible for.
///
/// It runs AFTER the original, so on any failure the namer is exactly the one the game built.
///
/// # Safety
///
/// `namer` is the object the original constructor just returned, and `base` the live module base.
/// The three path calls write only into the caller's buffers; the push takes the accessor by
/// reference the same way the original loop does.
unsafe fn add_spare_cell(base: usize, namer: usize) -> bool {
    if namer == 0 {
        return false;
    }
    let refuse = |why: core::fmt::Arguments<'_>| {
        log(format_args!("{LOG_PREFIX} cell REFUSED {why}"));
        false
    };
    ...
    if count != ds2_rva::FE_QUIT_TAB_CELL_IDS.len() {
        return refuse(format_args!("count={count}, expected 3"));
    }
    ...
    // constructor put there. This is the check a byte-diff could not be: the slack between the ids
    // and the length is uninitialised stack and differs between entries the game built back to
    // back, so only the named fields mean anything.
    let last = entry(count - 1);
    let base_path = ds2_rva::FE_QUIT_TAB_BASE_PATH;
    for (i, want) in base_path.iter().enumerate() {
    ...
    if got_len != ds2_rva::FE_SCENE_NAMER_ENTRY_LEN {
        return refuse(format_args!(
            "entry[{}] length is {got_len}, expected 5",
            count - 1
        ));
    }

    // THREE CLONES, THREE DISTINGUISHABLE ANSWERS. Capacity is six and three are used, so three is
    // the budget; spending it on one guess would waste two thirds of it. Each clone is byte-identical
    // to the shipped Quit row except for the one or two fields named, so a clone that fails to
    // resolve says something about the ID, not about the shape of the object.
    //
    // `row-extent` on the tab report then names the winner: 4 means the first resolved, 5 the
    // second, 6 the third, and 3 means the spare cells are not reachable this way at all.
    const CLONES: [(u32, u32); 3] = [
        (
            ds2_rva::FE_QUIT_TAB_BASE_PATH[3],
            ds2_rva::FE_QUIT_TAB_SPARE_CELL_IDS[0],
        ),
        (
            ds2_rva::FE_QUIT_TAB_BASE_PATH[3],
            ds2_rva::FE_QUIT_TAB_SPARE_CELL_IDS[1],
        ),
        (
            ds2_rva::FE_QUIT_TAB_CACHE_CONTAINER,
            ds2_rva::FE_QUIT_TAB_SPARE_CELL_IDS[0],
        ),
    ];
    ...
    for (slot, (container, id)) in CLONES.iter().enumerate() {
        let at = count + slot;
        // SAFETY: `at < capacity` is guaranteed by count==3 and CLONES.len()==3.
        unsafe { push(list as *mut u8, last) };
        // SAFETY: the push just made element `at` live, and both offsets are inside it.
        unsafe {
            let clone = entry(at);
            clone
                .add(ds2_rva::FE_SCENE_NAMER_ENTRY_CONTAINER_OFFSET)
                .cast::<u32>()
                .write(*container);
            clone
        ...
        log(format_args!(
            "{LOG_PREFIX} cell-clone row={at} from=entry[{}] container={container:#x} id={id:#x}",
            count - 1
        ));
    }
    let n = CELLS_ADDED.fetch_add(1, Ordering::Relaxed) + 1;
    log(format_args!(
        "{LOG_PREFIX} cell-added namer=0x{namer:016x} count={count}->{} additions={n} \
         -- row-extent 4/5/6 names which clone resolved, 3 means none did",
        count + CLONES.len()
    ));
    true
}
...
unsafe extern "system" fn namer_detour(out: *mut usize, allocator: *mut u8) -> *mut usize {
    ...
    let base = ds2_game_base::mem::game_module_base().unwrap_or(0);
    if base != 0 {
        unsafe { add_spare_cell(base, namer) };
    }
    returned
}
...
pub unsafe fn install() -> Outcome {
    ...
    // THE FOURTH CELL. Installed after the dispatch and before the probe: without it the appended
    // item has no cell and is invisible, which is the whole defect this crate started as.
    ...
    // THE PROBE, installed fourth and never fatal. It only reads, and it reports for all SIX tabs
