...
//! points the game's own save-directory builder at a staged copy of whatever file you name. So the
//! row STAGES the picked file and then asks the game to go back to the title screen, where LOAD
//! GAME reads the staged copy. The player's own save is never opened, and the staged copy is what
//! a redirected run writes into.
//!
