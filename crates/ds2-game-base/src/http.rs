...
//! standalone probe built exactly like this completed the full TLS handshake inside a Proton
//! prefix, returning HTTP 200 and the expected body with no winetricks, no CA bundle and no
//! native override. That measurement is the only reason this module exists in this shape.
//!
//! **The measurement was taken in Elden Ring's prefix** (`compatdata/1245620/pfx`) in
//! `../er-mods-rs`, and has NOT been repeated in this game's prefix. What it establishes is
//! that Wine's `winhttp` does real TLS -- a property of the Wine build, not of the game -- so
//! it is good evidence and not proof. Treat the first request this crate makes from inside DS2
//! as the measurement that settles it.
//!
...
//! and cannot drift with a crate upgrade. That also keeps tier A at zero dependencies.
...
///
/// [`MAX_BODY_BYTES`] suits a document of a few kilobytes. A caller that legitimately expects
/// megabytes -- a paginated API listing, say -- states its own limit here rather than raising
/// the default for everyone and losing the protection the cap exists to give.
///
/// Blocking, with the same thread rules as [`get`].
///
/// # Errors
///
...
/// GET `https://{host}{path}` and return the body, capped at [`MAX_BODY_BYTES`].
///
/// BLOCKING, on the calling thread, for as long as the network takes. Must not be called from
/// `DllMain` -- it would run under the loader lock, and WinHTTP loads DLLs -- and must not be
/// called from a frame callback or any other hook that runs on the game thread, where it would
/// stall the game for the duration of a TLS handshake. Run it on a worker thread.
///
/// # Errors
///
/// Returns [`HttpError`] identifying the step that failed.
...
fn request(
    ...
    unsafe {
        ...
        // Any 2xx, not just 200. A create endpoint answers 201, and treating that as a failure
        // would report a failed upload that in fact stored the thing.
