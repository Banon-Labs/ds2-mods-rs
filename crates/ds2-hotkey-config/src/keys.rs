//! One table of key names, carrying BOTH codes a key can reach a Windows game process through.
//!
//! # Why one table and not two
//!
//! A key reaches this process by two different routes and is numbered differently on each. A
//! `GetAsyncKeyState` poll speaks Win32 VIRTUAL KEYS (`VK_F7 == 0x76`); a DirectInput keyboard
...
//! a key from a DirectInput buffer while ALSO polling it needs both numbers for the same key.
//!
//! Keeping only the half a given DLL happens to need is what produces two mods in one install that
//! spell the same key differently -- one knowing `"f7"` as a scancode, the other `"F7"` as a
//! virtual key, with their own spellings for the numeric keypad and no way to ask either table a
//! question in the other's terms. [`NAMED_KEYS`] carries both codes per row, so a config file has
//! ONE vocabulary regardless of which route the DLL reading it uses.
//!
//! # Why names and not numbers
//!
//! The alternative is a hard-coded key (`VK_INSERT`, `VK_F7`). A 60% keyboard -- the common compact
//! EN-US layout -- has no Insert and no function row, which locks whole features out for anyone
//! using one, and two mods that both picked F7 fight over it with no way for the player to separate
...
//! # Aliases
//!
//! The table is deliberately generous: several spellings resolve to the same key, so a player who
//! writes what they already write elsewhere gets the key they meant instead of a parse error. That
//! is why the keypad has four names per key (`kp_plus`, `numpad_plus`, `numpad_add`, `numpad_+`)
//! and why both `lbracket` and `leftbracket` resolve. A vocabulary that grows one alias at a time,
//! after each player reports it missing, is the same feature arriving late and looking broken in
//! the meantime.
//!
//! # No game knowledge
//!
//! Nothing below is specific to DARK SOULS II or to any other game. These are Win32 and
//! DirectInput constants, published and stable for decades; the crate builds and its tests run on
//! a Linux host precisely because there is nothing here to run against.
...
pub const NAMED_KEYS: &[NamedKey] = &[
    ...
    // Numeric keypad. `kp_*` is this crate's canonical spelling; `numpad*` is what most other
    // mods' config files say and what a player is likely to write, so both resolve.
...
mod tests {
    ...
    /// The spellings a hotkey default is actually written in all parse. Every one of these is a
    /// key some mod picks as its out-of-the-box binding, in the form a config file writes it.
    #[test]
    fn the_spellings_a_default_is_written_in_all_parse() {
        ...
        // A modified letter, resolved in scancode space by a DLL reading a DirectInput buffer.
        assert_eq!(
            parse_scancode_chord("ctrl+alt+c").map(|c| c.dik),
            Ok(Some(0x2e))
        );
        // Keypad, in the `numpad*` spelling.
        ...
        // The same keypad key in the canonical spelling, and a bracket alias.
