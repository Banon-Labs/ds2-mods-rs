...
//!
//! The obvious cheap check is the modification time, and it is what a config watcher usually
//! reaches for first. It has a resolution problem: several filesystems -- and every network/VM
//! share a Wine or Proton prefix is likely to sit on -- stamp mtime to a whole second. Save a file twice inside one second and the second save
...
impl HotFile {
    ...
    ///
    /// A DLL that rewrites its own config -- appending to a list it owns, rewriting a line it just
    /// changed -- would otherwise see its own write on the next poll as text it has not seen
    /// before, report a reload, and reset the key edge detectors mid-keypress.
...
mod tests {
    ...
    fn a_rewrite_with_identical_text_is_not_a_change() {
        let mut hot = HotFile::with_interval("ds2-test.toml", 0);
    ...
    fn the_real_poll_reads_the_real_file() {
        let dir = std::env::temp_dir().join(format!("ds2-hotkey-config-{}", std::process::id()));
